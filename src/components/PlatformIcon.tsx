function FacebookIcon() {
  return (
    <svg viewBox="0 0 100 100" className="h-16 w-16" fill="none">
      <circle cx="50" cy="50" r="41" stroke="white" strokeWidth="5" />
      <path
        d="M58 33h-7c-6.6 0-11 4.4-11 11v7h-8v11h8v23h12V62h9l1.5-11H52v-6c0-3 1-5 5-5h6z"
        fill="white"
      />
    </svg>
  );
}

function InstagramIcon() {
  return (
    <svg viewBox="0 0 100 100" className="h-16 w-16" fill="none">
      <rect x="15" y="15" width="70" height="70" rx="20" stroke="white" strokeWidth="5" />
      <circle cx="50" cy="50" r="17" stroke="white" strokeWidth="5" />
      <circle cx="71" cy="29" r="4" fill="white" />
    </svg>
  );
}

function TikTokIcon() {
  return (
    <svg viewBox="0 0 100 100" className="h-16 w-16" fill="none">
      <path
        d="M62 18v42a15 15 0 1 1-15-15"
        stroke="white"
        strokeWidth="6"
        strokeLinecap="round"
      />
      <path
        d="M62 18c1.5 11 10 18.5 21 19.5"
        stroke="white"
        strokeWidth="6"
        strokeLinecap="round"
      />
    </svg>
  );
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 48 48" className="h-16 w-16">
      <path
        fill="#4285F4"
        d="M45.12 24.5c0-1.56-.14-3.06-.4-4.5H24v8.51h11.84c-.51 2.75-2.06 5.08-4.39 6.64v5.52h7.11c4.16-3.83 6.56-9.47 6.56-16.17z"
      />
      <path
        fill="#34A853"
        d="M24 46c5.94 0 10.92-1.97 14.56-5.33l-7.11-5.52c-1.97 1.32-4.49 2.1-7.45 2.1-5.73 0-10.58-3.87-12.31-9.07H4.34v5.7C7.96 41.07 15.4 46 24 46z"
      />
      <path
        fill="#FBBC05"
        d="M11.69 28.18A13.44 13.44 0 0 1 11 24c0-1.45.25-2.86.69-4.18v-5.7H4.34A21.98 21.98 0 0 0 2 24c0 3.55.85 6.91 2.34 9.88l7.35-5.7z"
      />
      <path
        fill="#EA4335"
        d="M24 10.75c3.23 0 6.13 1.11 8.41 3.29l6.31-6.31C34.91 4.18 29.93 2 24 2 15.4 2 7.96 6.93 4.34 14.12l7.35 5.7c1.73-5.2 6.58-9.07 12.31-9.07z"
      />
    </svg>
  );
}

export default function PlatformIcon({ platform }: { platform: string }) {
  switch (platform) {
    case "Facebook":
      return <FacebookIcon />;
    case "Instagram":
      return <InstagramIcon />;
    case "TikTok":
      return <TikTokIcon />;
    case "Google Review":
      return <GoogleIcon />;
    default:
      return <FacebookIcon />;
  }
}

export function TapPhoneIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 120 100"
      className={className ?? "h-12 w-24"}
      fill="none"
      stroke="currentColor"
      strokeWidth="4"
    >
      <rect x="68" y="16" width="32" height="56" rx="7" />
      <line x1="84" y1="60" x2="84" y2="60.5" strokeLinecap="round" strokeWidth="6" />
      <path d="M52 32a24 24 0 0 1 0 36" strokeLinecap="round" />
      <path d="M40 22a44 44 0 0 1 0 56" strokeLinecap="round" />
      <path d="M28 12a64 64 0 0 1 0 76" strokeLinecap="round" />
    </svg>
  );
}
